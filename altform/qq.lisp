...
;;;; hacked up version of quasiquote appendix from cltl2


;;;; trying to work in quasiquoting of non-list structures,
;;;; aka altform.lisp.  modified bq-process below, straight
;;;; unquoting seems to be working ok, unquote-splicing is
;;;; not there yet. Haven't fully read through the code yet,
;;;; will probably become clear after doing so.
...
(defun bq-process-altform (x new)
  (declare (ignore x))
  `(apply ',(car new) ,(bq-completely-process (cdr new))))

(defun bq-process (x)
  (let ((new (transform x nil)))
    (cond ((not (eq x new))
           (bq-process-altform x new))
          ((atom x) 
           (list *bq-quote* x)) 
          ((eq (car x) 'backquote) 
